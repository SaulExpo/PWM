...
async function addFavourites(user, film, serie) {
    try {
        ...
        if (favourites == null){
            createFavourites(user, film, serie);
        }
        ...
        if (serie == null){
            ...
            let existingFilms = favourites.series.map(f => ({ id: f.id })); // Películas ya en favoritos
...
async function removeFavourite(user, film, serie) {
    try {
        ...
        if(serie == null){
            ...
            const existingFilms = favourites.series.map(f => ({ id: f.id })); // Películas ya en favoritos
            // Verificar si la película está en favoritos
            console.log(existingFilms);
            if (!existingFilms.some(f => f.id === serie.id)) {
                console.log("La película no está en favoritos.");
